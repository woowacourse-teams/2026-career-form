...
일반 작업은 `develop`에서 시작한다. Start release는 현재 `develop` HEAD에서
...
| `CF-*` → `main` | Squash Merge | `hotfix` 라벨이 있는 운영 긴급 수정 |
| `main` → `develop` | Merge Commit | `hotfix` 라벨이 있는 운영 수정 동기화 |
| `main` → `release/*` | Merge Commit | `hotfix` 라벨이 있는 활성 릴리스 동기화 |
| `revert/*` → `main` | Merge Commit | 운영 배포 실패 되돌림 |

`develop` → `main`과 hotfix 조건 없는 `CF-*` → `main`, `main` 동기화는
허용하지 않는다. `CF-*` 작업 PR은 연결 Issue 하나를 종료하고 Issue와 같은 제목을
사용한다. release·동기화·revert 시스템 PR은 `[Release] 작업명` 제목을 사용하고
Issue를 종료하지 않는다.

## hotfix와 되돌림

`CF-*` → `main`은 PR 또는 연결 Issue에 `hotfix` 라벨이 있을 때만 허용한다.
운영 반영 후 `main` → `develop`과, 활성 release가 있으면 `main` → `release/*`
동기화 PR을 각각 만들고 PR에 `hotfix` 라벨을 붙인다.
...
막고 승인 리뷰 1개를 요구한다. 하네스는 경로와 라벨 계약을 검사하지만 실제 병합
