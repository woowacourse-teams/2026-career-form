...
|---|---|---|
| `status:planning` | `In Progress` | 계약 작성 또는 GitHub 수동 편집 중 |
| `status:ready` | `In Progress` | 사람이 계약을 승인하고 구현 대기 |
| `status:in-progress` | `In Progress` | 구현, 자동 검증 또는 Draft PR 수동 편집 중 |
| `status:blocked` | `In Progress` | 작업 식별자를 유지한 채 사람 판단 대기 |
| `status:review` | `On Review` | Draft PR 검토 중 |

Project draft를 Issue로 승격한 직후 Project Status는 `In Progress`로 바꾼다. AI가 Issue 계약 초안을 게시한 뒤 사람이 GitHub에서 수정하고 재개할 때까지 `status:planning`을 유지한다. 재개 후 원격 계약 검증을 통과하면 `status:ready`로 바꾼다. 두 라벨을 Project Status `Todo`로 동기화하지 않는다. 실제 구현을 시작하면 `status:in-progress`로 전환하고 Draft PR 수동 편집이 끝날 때까지 유지한다.

## 연결 계약

Issue와 PR 제목은 같은 `[영역] 작업명`을 사용하며 `FE`, `BE`, `AI`, `Infra`,
`Harness`, `Plan` 영역을 허용한다. `FE`, `BE`, `AI`만 전체를 대문자로 쓰고 나머지는 첫
문자만 대문자로 쓴다. `AI`는 제품의 LLM, 모델, 프롬프트, 에이전트 기능 작업에 사용하고
`Harness`는 개발 하네스와 워크플로우 변경에 사용한다. `Plan`은 조사, 요구사항 정리, 문서 기획 같은
...
3. Issue 본문과 단일 PR의 논리적 커밋 계획을 작성해 원격 Issue에 게시한다.
4. 사람이 GitHub Issue를 수정하고 재개하면 AI가 수정된 원격 계약을 검증한 뒤 `status:ready`로 전환한다.
5. 구현 시작 시 `status:in-progress`로 바꾸고 계약에 맞는 `CF-<Issue 번호>` 또는 `hotfix/CF-<Issue 번호>` 브랜치를 만든다.
6. TDD, 전체 검증, 두 축 코드 리뷰를 수행한다.
7. Issue와 같은 제목의 Draft PR을 만들고 `status:in-progress`와 Project `In Progress`를 유지한다.
8. 사람이 GitHub Draft PR을 수정하고 재개하면 AI가 기존 원격 PR을 검증한 뒤 `status:review`와 Project `On Review`로 전환한다.
9. 사람이 Draft PR을 Ready for review로 전환한다.
10. 사람이 최종 승인, Squash Commit 제목 입력, 머지를 수행한다.
11. AI가 PR의 `MERGED` 상태와 merge commit의 `origin/<대상 브랜치>` 포함 관계를 증명한다.
12. clean한 관리 worktree와 안전한 로컬 브랜치만 정리하고 dirty 또는 외부 관리 worktree는 보존한다.
