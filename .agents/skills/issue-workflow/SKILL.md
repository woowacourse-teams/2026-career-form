...
4. Issue가 열려 있고 `status:ready` 라벨, `[영역] 작업명` 제목, 필수 계약 섹션을 갖췄는지 `harness/scripts/validate-issue.py`와 같은 기준으로 검사한다.
...
4. 파일 수정과 원격 상태 변경 전에 `harness/scripts/ensure-environment.py`를 실행한다. 이 진입점은 `doctor.py`로 현재 상태를 확인하고, 필요한 경우에만 `bootstrap.py`를 실행한 뒤 `doctor.py`로 다시 검증한다.
...
1. 관련 테스트를 실행한다.
2. `harness/scripts/verify.py`를 실행한다.
