...
하나의 Issue, 계약에 맞는 `CF-<Issue 번호>` 또는 `hotfix/CF-<Issue 번호>` 브랜치 하나, PR 하나의 생명주기를 단계별 정본에 따라 연결한다. Issue와 Draft PR의 GitHub 수정, 사람 승인과 머지 대기에서는 멈추고, 같은 스킬을 다시 호출하면 원격 상태를 읽어 첫 미완료 단계부터 재개한다.
