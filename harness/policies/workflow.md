...
2. AI가 draft 제목을 `[영역] 작업명`으로 검사하고 필요한 경우 보정한 뒤 Issue로 승격한다. 영역은 `FE`, `BE`, `Infra`, `Harness`, `PLAN`을 허용한다. Issue에는 `status:planning`, Project에는 In Progress를 적용하고 함께 확인한다.
