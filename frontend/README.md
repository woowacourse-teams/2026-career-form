...
처음 설치하면 확장 내부 `onboarding.html` 안내가 새 탭에서 열립니다. 업데이트 시에는 열리지 않습니다. 안내의 프로필 관리 버튼은 실제 options 페이지를 엽니다. 자동 기입 시 현재 입력칸만 강조하고, 결과의 확인 필요 항목에서 같은 페이지의 필드 위치로 이동할 수 있습니다.
