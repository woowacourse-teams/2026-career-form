...
처음 설치하면 확장 내부 `onboarding.html` 안내가 새 탭에서 열립니다. 업데이트 시에는 열리지 않습니다. PR #97의 웹 온보딩을 공유하되 설치 후 진입점은 재설치 안내 대신 커리어폼 열기부터 시작합니다. 웹에서 직접 연 사용 안내에는 설치 링크를 유지합니다. 넓은 화면에서는 안내 그림과 설명을 나란히 배치하고 좁은 화면에서는 세로로 표시합니다. 실제 프로필 등록은 확장 패널에서 진행합니다. 자동 기입 시 현재 입력칸만 강조하고, 결과의 확인 필요 항목에서 같은 페이지의 필드 위치로 이동할 수 있습니다.
...
5. HTTP(S) 테스트 페이지에서 확장 프로그램 아이콘을 눌러 페이지 안 지원서 패널이 바로 열리는지 확인합니다. Chrome 설정이나 확장 내부 페이지처럼 실행할 수 없는 화면에서는 지원서로 이동하라는 안내가 열립니다.
