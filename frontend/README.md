...
처음 설치하면 확장 내부 `onboarding.html` 안내가 새 탭에서 열립니다. 업데이트 시에는 열리지 않습니다. 안내는 PR #97의 웹 온보딩 문구, 버튼, 3단계 구성과 스타일을 그대로 사용하며 내부 페이지 경로만 확장 패키지로 연결합니다. 실제 프로필 등록은 원본 안내에 따라 확장 패널에서 진행합니다. 자동 기입 시 현재 입력칸만 강조하고, 결과의 확인 필요 항목에서 같은 페이지의 필드 위치로 이동할 수 있습니다.
