...
describe("side panel launcher visibility", () => {
  it("shows on application URLs and SK Careers application pages", () => {
    ...
    expect(shouldShowSidePanelLauncher(new URL("https://www.skcareers.com/Application/Index/R261876"))).toBe(true);
  });
  ...
  it("hides on pages whose URL does not contain apply", () => {
    ...
    expect(shouldShowSidePanelLauncher(new URL("https://recruit.skhynix.com/job/list"))).toBe(false);
    expect(shouldShowSidePanelLauncher(new URL("https://www.skcareers.com/Recruit"))).toBe(false);
  });
});
