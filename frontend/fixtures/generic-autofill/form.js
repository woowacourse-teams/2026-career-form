(() => {
  ...
  const rows = () => [
    ...(list?.querySelectorAll("[data-repeater-item]") ?? []),
  ];
  const refreshCount = () => {
    if (count) count.textContent = `행 ${rows().length}개`;
  };
  ...
  const revealMilitaryDetails = () => {
    const selected = form?.querySelector(
      "input[name='military.military.militaryStatus']:checked",
    );
    const details = document.querySelector("#military-details");
    const controls =
      form?.querySelectorAll("input[aria-controls='military-details']") ?? [];
    if (!details) return;
  ...
  const setupCombobox = () => {
    ...
    const filter = () => {
      ...
      options().forEach((option) => {
        const matches =
          !query ||
          option.textContent.toLocaleLowerCase("ko-KR").includes(query);
  ...
  document
    .querySelector("#add-certification")
    ?.addEventListener("click", addRow);
  document
    .querySelectorAll("input[aria-controls='military-details']")
    .forEach((control) =>
      control.addEventListener("change", revealMilitaryDetails),
    );
  ...
  document.querySelector("#demo-rerender")?.addEventListener("click", () => {
    ...
    replacement.querySelector("[data-row-title]").textContent =
      "자격증 행 1 (새 DOM)";
    ...
    replacement.querySelector("[data-name-label]").htmlFor =
      "cert-name-rerendered";
    replacement.querySelector("[data-date-label]").htmlFor =
      "cert-date-rerendered";
  ...
  form?.addEventListener("submit", (event) => {
    event.preventDefault();
    announce("합성 fixture는 제출하지 않습니다.");
  });
  refreshCount();
  setupCombobox();
})();
