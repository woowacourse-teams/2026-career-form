...
- 위 두 실행은 실제 브라우저에서 합성 입력 대상과 공개 검색을 연결한 검증이며, 설치된 확장의 전체 패널 성공이나 저장·제출 호환성, CJ 데이터베이스 전체의 유일성을 입증한 것은 아닙니다.
- 이후 설치된 확장의 실제 화면 실행에서 주전공 표시값과 공개 결과 코드의 일치, 팝업 종료, 578ms 구간의 유지도 읽기 전용으로 확인했습니다. 그러나 결과 UI는 해당 항목을 `입력 결과 확인`으로 분류하고 입력 완료를 0개로 표시했습니다. 따라서 전체 패널 성공으로 집계하지 않습니다. 기존 `result-model.ts`는 `LLM_SUGGESTED` 쓰기를 보수적으로 확인 필요로 분류하지만, 이 실행의 내부 매핑 상태나 실행기 반환값은 직접 수집하지 않았습니다.
- 실제 패널의 자동 기입은 항목 선택 단계 없이 실행되어 다른 날짜 항목도 입력됐습니다. 전공만 실행하거나 실제 화면의 다른 입력값 불변성을 검증한 것으로 보고하지 않습니다. 추가 입력을 중단했으며 저장·제출·페이지 이동·미리보기는 실행하지 않았습니다. 실제 입력값과 계정·세션 정보는 이 기록에 포함하지 않습니다.

## CF-112 첫 대학교 학교명·국가 문맥 검색 경계

CJ의 실제 공개 학교 팝업 구조를 **비식별 합성 응답**으로 재현한
[`cj-school-public-response.fixture.html`](../../src/autofill/interaction/cj-school-public-response.fixture.html)은
POST `school_name`과 `num=2_0`, 표시값·학교 코드·국가 코드의 결과 literal 및 닫기 스크립트를 담습니다.
예시 학교명·코드는 합성값이며 계정, 지원서 입력값 및 세션 식별자를 포함하지 않습니다.
완전히 수신된 검증 응답의 정확 일치 하나는 렌더링 목록 안의 유일성이며 DB 전체 유일성은 아닙니다.
...
### CF-112 격리 브라우저 하네스와 검증 기록 (2026-09-26)
...
실제 브라우저에서는 공개 학교 검색 탭의 문서를 **독립된 합성 HTML**로 교체했습니다. 실제 지원서 문서에는 장착하지 않았습니다. 이 문서에 공개 `needPopup.js`가 로드되고 검토된 메서드 가드가 `ready`를 보고한 뒤, 신뢰된 `/tmp/cf112-school-harness.js` IIFE를 주입했습니다.
- 첫 실행은 `selected` / `value-observed`, 832ms 경과, 팝업 종료 후 549ms 유지였고 표시값, 비어 있지 않은 코드, 국가 변경, 소재지 opener 국가 URL, 기존 `new_country`, 빈 소재지, 무관 필드, 팝업 종료의 각 검사와 `pass`가 모두 참이었습니다.
- 하네스 UI로 초기화한 뒤 두 번째 실행도 `selected`, 834ms 경과, 533ms 유지였으며 같은 항목의 검사가 모두 참이었습니다. 이 두 실행은 공개 학교 검색 endpoint와 실제 공개 팝업 라이브러리를 합성 대상에 연결한 결과입니다.
- 별도의 새 공개 기본 검색에서 같은 비개인 검색어의 정확 일치 결과 literal을 읽고, 결과 코드를 하네스가 기록한 코드와 독립 대조하여 같음을 확인했습니다. 결과 링크는 누르지 않았습니다. 공개 코드를 문서에 기록하지 않습니다.
- 독립 Spec 및 Standards 검토는 `31c7e78` 기준으로 남은 조치 사항이 없었습니다. 이 변경은 회사 전용 어댑터가 아니라 범용 상호작용 계약이므로 회사 어댑터 문서 신설은 범위 밖입니다.
- 설치 확장 전체 패널과 실제 지원서에서의 실행은 검증하지 않았고, 저장·제출은 하지 않았습니다. 이 격리 결과를 실제 지원서 호환성으로 일반화하지 않습니다.
