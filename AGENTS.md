...
- AI는 새 clone 또는 worktree에서 파일을 수정하기 전에 `harness/scripts/ensure-environment.py`로 작업 환경을 자동 구성한다. 자동 구성에 실패하면 수정하지 않고 원인을 보고한다.
...
- 완료 전 `harness/scripts/verify.py`를 실행하고 최신 결과를 PR에 기록한다.
