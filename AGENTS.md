...
- 작업 계획은 `git rev-parse --git-path cf-workflow/plan.md`가 반환하는 worktree별
  Git 메타데이터 경로에 기록한다.
...
- 컨벤션과 Git 계약은 `llm-wiki/wiki/topics/project-conventions.md`를 따른다.
- 개별 커밋과 PR 제목은 서로 다른 형식을 사용한다.
...
- 현재 Issue 본문은 작업 범위와 완료 기준의 정본이고, 위치 의존적인 하네스 정책은
  해당 파일의 현재본이 우선한다. 그 밖의 제품 기준, ADR, 기술과 컨벤션 지식은
  topic Wiki에서 현재 상태를 찾고 연결된 불변 raw를 근거로 사용한다.
- 작업 중 발견한 비즈니스, 기술, 컨벤션 맥락은 worktree 체크포인트의 지식 후보로
  보존한다. raw 작성 직전에 후보 전체 또는 `No reusable knowledge`를 한 번에 제시하고
  사람 승인 digest를 받은 뒤에만 Issue raw와 topic Wiki에 반영한다.
...
1. `llm-wiki/wiki/topics/adapter-development.md`
2. `llm-wiki/wiki/topics/adapter-<company>.md`
3. `llm-wiki/wiki/topics/adapter-field-inventory.md`
