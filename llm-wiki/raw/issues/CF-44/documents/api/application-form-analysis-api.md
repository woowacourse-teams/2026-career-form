...
브라우저는 승인된 plan의 효과를 관측한 뒤 이전 DOM handle을 폐기하고 새 DOM을 수집한다. 새 화면에서도 또 준비가 필요하면 이 endpoint를 새 Snapshot A로 다시 호출한다. preparation request에 field를 섞거나 preparation response에 write plan을 넣지 않는다.
