...
동적 DOM은 클릭별 효과 검증과 재수집으로 안전하게 처리한다. 반복 행의 실제 실행 횟수와 안전 제한은 browser만 알며 backend와 LLM은 프로필 항목 개수도 받지 않는다. 로컬 안전 정책에 걸리거나 action 효과 확인·재탐색에 실패하면 소비자는 fields endpoint로 진행하지 않고 preparation endpoint에 새 Snapshot A를 보낸다.
