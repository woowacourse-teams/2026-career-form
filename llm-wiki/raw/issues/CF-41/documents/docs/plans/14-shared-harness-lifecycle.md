...
## 6. Issue와 PR 영역 prefix 표기 통일

- [x] 영역 prefix의 대소문자를 제한하는 실패 테스트를 추가한다.
- [x] 제목 검증기와 배포 PR 계약이 고정된 prefix 표기만 허용하게 한다.
- [x] 기획 스킬, 정책, 컨벤션과 예시 제목을 당시 계약에 맞춘다.
- [x] Issue #14와 PR #15 제목을 당시 계약에 맞춘다.
- [x] 관련 테스트와 전체 검증을 통과한다.

이 결정의 대소문자 규칙은 Issue #17에서 `FE`, `BE`만 전체 대문자를 사용하고 나머지는 첫 문자만 대문자를 사용하는 규칙으로 대체됐다.

커밋: `fix: Issue와 PR 영역 prefix 표기 강제`
