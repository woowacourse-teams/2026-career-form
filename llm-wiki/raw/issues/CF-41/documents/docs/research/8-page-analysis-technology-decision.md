...
조건부 채택. 자동 PoC 결과로 기술 방향을 정했고 삼성의 다섯 섹션에서 현재 표시 후보와 휴리스틱 추가 후보를 대조했다. 삼성의 동적 입력 영역·접근 불가 iframe과 SK·CJ 실제 페이지 검증이 남아 있다.
...
- 비활성 섹션의 입력 요소를 DOM에 숨겨 두는 사이트에서는 전체 후보 집계만으로 현재 화면 누락을 판정할 수 없어 섹션별 가시성 대조가 필요하다.
- `추가` 동작 뒤 입력 행을 생성하는 사이트에서는 숨겨진 DOM 집계만으로 후보를 발견할 수 없다. 저장을 일으키지 않는 표시 동작 전후의 구조 차이를 별도 수집해야 한다.
